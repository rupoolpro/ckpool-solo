		       ...
		       const bool submit)
{
	...
	LOGINFO("Client %s biased diff_coeff %.2f bias %.2f bdiff %.2f dsps %.2f dsps5 %.2f drr %.2f adjust diff from %"PRId64" to: %"PRId64" ",
		client->identity, diff_coeff, bias, bdiff, dsps, client->dsps5, drr, client->diff, optimal);
